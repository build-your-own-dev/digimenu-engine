...
import {
  ...
  FileImage, Minus, QrCode, RefreshCw, ScanText, Search, Settings, ShieldCheck, ShoppingBag, Smartphone, Sparkles, Trash2, UploadCloud, UserPlus, Users, UtensilsCrossed, X,
} from "lucide-react";
...
type RecognizedMenuItem = { id: string; category: string; name: string; description: string; price: string };
...
const menuCategoryWords=["aperitif","bier","burger","cocktails","dessert","desserts","drinks","empfehlungen","frühstück","getränke","hauptgerichte","kaffee","kinder","pasta","pizza","salate","snacks","speisen","spezialitäten","suppen","vorspeisen","wein"];
function cleanOcrLine(line:string){return line.replace(/[|]+/g," ").replace(/\s+/g," ").replace(/^[•·*—–-]+\s*/,"").trim()}
function looksLikeCategory(line:string){const normalized=line.toLocaleLowerCase("de-CH").replace(/[:\-–—]+$/g,"").trim();if(menuCategoryWords.some(word=>normalized===word||normalized.startsWith(`${word} `)))return true;const letters=line.replace(/[^A-Za-zÄÖÜäöüß]/g,"");return line.length<=42&&letters.length>=3&&line===line.toLocaleUpperCase("de-CH")}
function parseRecognizedMenu(text:string){
  const lines=text.split(/\r?\n/).map(cleanOcrLine).filter(line=>line.length>1);const found:RecognizedMenuItem[]=[];let category="Erkannte Speisen";
  const pricePattern=/(?:CHF|EUR|€|Fr\.?|SFr\.?)?\s*(\d{1,3}(?:[.,]\d{1,2})?)\s*(?:CHF|EUR|€|Fr\.?|SFr\.?|[.–—-])?\s*$/i;
  for(const line of lines){const match=line.match(pricePattern);if(!match&&looksLikeCategory(line)){category=line.replace(/[:\-–—]+$/g,"").replace(/\b\w/g,char=>char.toLocaleUpperCase("de-CH"));continue}if(match){const name=cleanOcrLine(line.slice(0,match.index)).replace(/[.·•\s]+$/g,"");const price=match[1].replace(",",".");if(name.length>=2&&name.length<=100&&Number.isFinite(Number(price)))found.push({id:crypto.randomUUID(),category,name,description:"",price});continue}const previous=found.at(-1);if(previous&&!previous.description&&line.length<=150&&!looksLikeCategory(line))previous.description=line}
  return found;
}
...
function Dashboard() {
  ...
  const [itemModal, setItemModal] = useState(false); const [categoryModal, setCategoryModal] = useState(false); const [recognitionModal,setRecognitionModal]=useState(false); const [qrModal, setQrModal] = useState(false);const[createModal,setCreateModal]=useState(false);const[switcherOpen,setSwitcherOpen]=useState(false);
  ...
  async function removeCategory(category:Category){const itemCount=items.filter(item=>item.category_id===category.id).length;const warning=itemCount?` Dabei werden auch ${itemCount} ${itemCount===1?"Eintrag":"Einträge"} gelöscht.`:"";if(!confirm(`Kategorie „${category.name}“ wirklich löschen?${warning}`))return;const{error}=await supabase!.from("categories").delete().eq("id",category.id);if(error){flash("Kategorie konnte nicht gelöscht werden");return}setCategories(current=>current.filter(entry=>entry.id!==category.id));setItems(current=>current.filter(item=>item.category_id!==category.id));flash("Kategorie gelöscht")}
  ...
  return <main className="dashboard">
    ...
    <section className={`dash-main ${dashboardSection==="menu"?"":"hidden-section"}`}>
      ...
      <div className="dash-toolbar"><div><h2>Menüeinträge <span>{items.length}</span></h2><p>Organisiere dein Angebot nach Kategorien.</p></div><div><button className="button button-outline" onClick={()=>setRecognitionModal(true)}><ScanText size={16}/> Speisekarte erkennen</button><button className="button button-outline" onClick={()=>setCategoryModal(true)}><Plus size={16}/> Kategorie</button><button className="button button-primary" onClick={()=>{setEditing(null);setItemModal(true)}}><Plus size={17}/> Eintrag hinzufügen</button></div></div>
      <div className="menu-board">{categories.length===0?<EmptyState onCategory={()=>setCategoryModal(true)} onRecognition={()=>setRecognitionModal(true)}/>:categories.map(cat=><section className="category-block" key={cat.id}><div className="category-title"><h3>{cat.name}</h3><div><span>{items.filter(i=>i.category_id===cat.id).length} Einträge</span><button className="category-delete" onClick={()=>void removeCategory(cat)} aria-label={`Kategorie ${cat.name} löschen`} title="Kategorie löschen"><Trash2 size={15}/></button></div></div><div className="item-table">{items.filter(i=>i.category_id===cat.id).map(item=><article className={!item.available?"unavailable":""} key={item.id}><div className={`item-thumb ${item.image_url?"has-image":""}`}>{item.image_url?<img src={item.image_url} alt=""/>:item.name.slice(0,1)}</div><div className="item-info"><b>{item.name}</b><span>{item.description || "Keine Beschreibung"}</span><div>{item.vegan&&<small>Vegan</small>}{item.vegetarian&&!item.vegan&&<small>Vegetarisch</small>}{item.spicy&&<small>Scharf</small>}</div></div><strong>{money(item.price,restaurant.currency)}</strong><label className="switch"><input type="checkbox" checked={item.available} onChange={()=>toggleAvailable(item)}/><span/></label><button className="icon-button" onClick={()=>{setEditing(item);setItemModal(true)}} aria-label="Bearbeiten"><Pencil size={16}/></button><button className="icon-button danger" onClick={()=>remove(item)} aria-label="Löschen"><Trash2 size={16}/></button></article>)}{items.filter(i=>i.category_id===cat.id).length===0&&<div className="empty-row">Noch keine Einträge in dieser Kategorie.</div>}</div></section>)}</div>
    </section>
    ...
    {recognitionModal&&<MenuRecognitionModal restaurant={restaurant} categories={categories} itemCount={items.length} onClose={()=>setRecognitionModal(false)} onSaved={()=>{setRecognitionModal(false);void loadMenu(restaurant);flash("Erkannte Speisekarte importiert")}}/>}
...
function MenuRecognitionModal({restaurant,categories,itemCount,onClose,onSaved}:{restaurant:Restaurant;categories:Category[];itemCount:number;onClose:()=>void;onSaved:()=>void}){
  const[files,setFiles]=useState<File[]>([]);const[inputVersion,setInputVersion]=useState(0);const[results,setResults]=useState<RecognizedMenuItem[]>([]);const[rawText,setRawText]=useState("");const[busy,setBusy]=useState(false);const[importing,setImporting]=useState(false);const[progress,setProgress]=useState(0);const[status,setStatus]=useState("");const[error,setError]=useState("");
  function chooseFiles(selected:FileList|null){setError("");if(!selected)return;const next=Array.from(selected).slice(0,5);const invalid=next.find(file=>!imageTypes.includes(file.type)||file.size>12*1024*1024);if(invalid){setError(`${invalid.name}: Bitte JPG, PNG oder WebP bis maximal 12 MB verwenden.`);setFiles([]);return}setFiles(next)}
  async function recognize(){if(files.length===0)return;setBusy(true);setError("");setResults([]);setRawText("");setProgress(1);setStatus("Texterkennung wird vorbereitet …");let worker:Awaited<ReturnType<typeof import("tesseract.js")["createWorker"]>>|null=null;const selected=[...files];try{const{createWorker}=await import("tesseract.js");let fileIndex=0;worker=await createWorker("deu",1,{logger:message=>{if(message.status==="recognizing text")setProgress(Math.max(3,Math.round(((fileIndex+message.progress)/selected.length)*100)))}});const recognized:RecognizedMenuItem[]=[];const textParts:string[]=[];for(fileIndex=0;fileIndex<selected.length;fileIndex+=1){setStatus(`Bild ${fileIndex+1} von ${selected.length} wird analysiert …`);const{data}=await worker.recognize(selected[fileIndex]);textParts.push(data.text);recognized.push(...parseRecognizedMenu(data.text))}setRawText(textParts.join("\n\n--- NÄCHSTES BILD ---\n\n"));setResults(recognized);setProgress(100);setStatus(recognized.length?`${recognized.length} Einträge erkannt. Bitte kurz prüfen.`:"Analyse abgeschlossen.");if(recognized.length===0)setError("Es wurden keine Einträge mit Preis erkannt. Nutze ein scharfes, gerade aufgenommenes Bild oder prüfe unten den erkannten Text.")}catch(recognitionError){setError(recognitionError instanceof Error?recognitionError.message:"Die Speisekarte konnte nicht erkannt werden.");setStatus("")}finally{if(worker)await worker.terminate();setFiles([]);setInputVersion(version=>version+1);setBusy(false)}}
  function updateResult(id:string,patch:Partial<RecognizedMenuItem>){setResults(current=>current.map(item=>item.id===id?{...item,...patch}:item))}
  async function importMenu(){const valid=results.filter(item=>item.name.trim()&&item.category.trim()&&Number.isFinite(Number(item.price))&&Number(item.price)>=0);if(valid.length!==results.length||valid.length===0){setError("Bitte fülle bei allen Einträgen Kategorie, Name und einen gültigen Preis aus.");return}setImporting(true);setError("");try{const categoryMap=new Map(categories.map(category=>[category.name.trim().toLocaleLowerCase("de-CH"),category.id]));const uniqueNames=Array.from(new Set(valid.map(item=>item.category.trim())));const missingNames=uniqueNames.filter(name=>!categoryMap.has(name.toLocaleLowerCase("de-CH")));if(missingNames.length){const{data:newCategories,error:categoryError}=await supabase!.from("categories").insert(missingNames.map((name,index)=>({restaurant_id:restaurant.id,name,sort_order:categories.length+index}))).select("id,name");if(categoryError)throw categoryError;for(const category of newCategories||[])categoryMap.set(category.name.trim().toLocaleLowerCase("de-CH"),category.id)}const payload=valid.map((item,index)=>({restaurant_id:restaurant.id,category_id:categoryMap.get(item.category.trim().toLocaleLowerCase("de-CH"))!,name:item.name.trim(),description:item.description.trim()||null,price:Number(item.price),available:true,vegetarian:false,vegan:false,spicy:false,image_url:null,sort_order:itemCount+index}));const{error:itemsError}=await supabase!.from("menu_items").insert(payload);if(itemsError)throw itemsError;onSaved()}catch(importError){setError(importError instanceof Error?importError.message:"Die erkannten Einträge konnten nicht importiert werden.")}finally{setImporting(false)}}
  return <div className="modal-backdrop recognition-backdrop" onMouseDown={event=>{if(event.currentTarget===event.target&&!busy&&!importing)onClose()}}><div className="modal recognition-modal"><div className="modal-head"><div><span className="section-kicker">LOKALE BILDERKENNUNG</span><h2>Speisekarte erkennen</h2></div><button className="icon-button" disabled={busy||importing} onClick={onClose} aria-label="Schliessen"><X/></button></div><div className="recognition-privacy"><ShieldCheck/><div><b>Das Bild bleibt privat.</b><p>Es wird nur in deinem Browser analysiert, nie zu Supabase hochgeladen und direkt nach der Analyse aus Menuva entfernt.</p></div></div>{results.length===0&&!busy&&<><label className="recognition-dropzone"><FileImage/><b>Speisekartenbilder auswählen</b><span>Bis zu 5 Bilder · JPG, PNG oder WebP · je maximal 12 MB</span><input key={inputVersion} type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={event=>chooseFiles(event.target.files)}/></label>{files.length>0&&<div className="recognition-files">{files.map(file=><span key={`${file.name}-${file.size}`}><FileImage/>{file.name}<small>{(file.size/1024/1024).toFixed(1)} MB</small></span>)}</div>}<button className="button button-primary button-full" disabled={files.length===0} onClick={()=>void recognize()}><ScanText size={17}/> {files.length?`${files.length} ${files.length===1?"Bild":"Bilder"} analysieren`:"Bilder auswählen"}</button></>}{busy&&<div className="recognition-progress"><div><Loader2 className="spin"/><b>{status}</b><strong>{progress}%</strong></div><span><i style={{width:`${progress}%`}}/></span><p>Je nach Gerät und Bildgrösse kann das kurz dauern.</p></div>}{results.length>0&&!busy&&<><div className="recognition-summary"><CircleCheck/><div><b>{results.length} Menüeinträge erkannt</b><span>Korrigiere die Felder bei Bedarf vor dem Import.</span></div><button className="button button-outline" onClick={()=>{setResults([]);setRawText("");setStatus("")}}>Andere Bilder</button></div><div className="recognition-table"><div className="recognition-table-head"><span>Kategorie</span><span>Gericht & Beschreibung</span><span>Preis ({restaurant.currency})</span><span/></div>{results.map(item=><article key={item.id}><input list="recognized-categories" aria-label="Kategorie" value={item.category} onChange={event=>updateResult(item.id,{category:event.target.value})}/><div><input aria-label="Gericht" value={item.name} maxLength={100} onChange={event=>updateResult(item.id,{name:event.target.value})}/><input aria-label="Beschreibung" value={item.description} maxLength={150} placeholder="Beschreibung (optional)" onChange={event=>updateResult(item.id,{description:event.target.value})}/></div><input aria-label="Preis" type="number" min="0" step="0.05" value={item.price} onChange={event=>updateResult(item.id,{price:event.target.value})}/><button className="icon-button danger" onClick={()=>setResults(current=>current.filter(entry=>entry.id!==item.id))} aria-label={`${item.name} entfernen`}><Trash2 size={15}/></button></article>)}</div><datalist id="recognized-categories">{categories.map(category=><option value={category.name} key={category.id}/>)}</datalist><div className="recognition-actions"><button className="button button-outline" onClick={onClose}>Abbrechen</button><button className="button button-primary" disabled={importing||results.length===0} onClick={()=>void importMenu()}>{importing?<Loader2 className="spin" size={17}/>:<Download size={17}/>} {results.length} Einträge importieren</button></div></>}{error&&<p className="form-message recognition-error">{error}</p>}{rawText&&<details className="recognition-raw"><summary>Erkannten Rohtext anzeigen</summary><pre>{rawText}</pre></details>}</div></div>
}
...
function PublicMenu({slug}:{slug:string}) {
  ...
  const [remembered,setRemembered]=useState<Record<string,number>>(()=>{if(typeof window==="undefined")return{};try{return JSON.parse(localStorage.getItem(`menuva-list-${slug}`)||"{}")}catch{return{}}});const[listOpen,setListOpen]=useState(false);
  ...
  useEffect(()=>{localStorage.setItem(`menuva-list-${slug}`,JSON.stringify(remembered))},[remembered,slug]);
  ...
  const rememberedItems=items.filter(item=>(remembered[item.id]||0)>0);const rememberedCount=rememberedItems.reduce((sum,item)=>sum+remembered[item.id],0);const rememberedTotal=rememberedItems.reduce((sum,item)=>sum+item.price*remembered[item.id],0);
  ...
  return <main className="public-menu" style={{"--accent":accent,"--accent-deep":accentDeep,"--accent-soft":accentSoft,"--accent-wash":accentWash,"--accent-contrast":accentContrast} as React.CSSProperties}>
    ...
    <section className="menu-content">{categories.filter(c=>active==="all"||active===c.id).map(category=>{const group=visible.filter(i=>i.category_id===category.id);if(!group.length)return null;return <section key={category.id} className="public-category"><div className="public-category-head"><span>{String(category.sort_order+1).padStart(2,"0")}</span><h2>{category.name}</h2></div><div className="public-items">{group.map(item=><article key={item.id} className={item.image_url?"with-image":""}>{item.image_url&&<img className="public-item-image" src={item.image_url} alt={item.name}/>}<div><div className="public-item-title"><h3>{item.name}</h3><strong>{money(item.price,restaurant.currency)}</strong></div><p>{item.description}</p><div className="public-item-meta"><div className="diet-tags">{item.vegan&&<span><Leaf size={12}/> Vegan</span>}{item.vegetarian&&!item.vegan&&<span><Leaf size={12}/> Vegetarisch</span>}{item.spicy&&<span>🌶 Scharf</span>}</div><button className={`remember-button ${remembered[item.id]?"remembered":""}`} onClick={()=>toggleRemembered(item.id)}><ShoppingBag size={14}/>{remembered[item.id]?"Gemerkt":"Merken"}</button></div></div></article>)}</div></section>})}{visible.length===0&&<div className="no-results"><Search/><h3>Nichts gefunden</h3><p>Versuch es mit einem anderen Suchbegriff.</p></div>}</section>
    {rememberedCount>0&&<button className="remembered-floating" onClick={()=>setListOpen(true)}><span><ShoppingBag size={18}/><b>Meine Merkliste</b></span><em>{rememberedCount}</em><strong>{money(rememberedTotal,restaurant.currency)}</strong></button>}
    {listOpen&&<div className="remembered-backdrop" onMouseDown={event=>{if(event.currentTarget===event.target)setListOpen(false)}}><aside className="remembered-panel"><header><div><span>FÜR DEINE BESTELLUNG</span><h2>Meine Merkliste</h2><p>Zeige diese Liste beim Bestellen oder lies sie einfach ab.</p></div><button onClick={()=>setListOpen(false)} aria-label="Merkliste schliessen"><X/></button></header><div className="remembered-list">{rememberedItems.map(item=><article key={item.id}>{item.image_url?<img src={item.image_url} alt=""/>:<div className="remembered-placeholder">{item.name.slice(0,1)}</div>}<div><b>{item.name}</b><small>{money(item.price,restaurant.currency)} pro Stück</small></div><div className="amount-control"><button onClick={()=>changeAmount(item.id,-1)} aria-label="Menge verringern"><Minus/></button><strong>{remembered[item.id]}</strong><button onClick={()=>changeAmount(item.id,1)} aria-label="Menge erhöhen"><Plus/></button></div><em>{money(item.price*remembered[item.id],restaurant.currency)}</em></article>)}</div><footer><div><span>Gesamtsumme</span><strong>{money(rememberedTotal,restaurant.currency)}</strong></div><button className="button button-outline" onClick={()=>{const text=rememberedItems.map(item=>`${remembered[item.id]}× ${item.name}`).join("\n");void navigator.clipboard.writeText(`${restaurant.name}\n${text}\nTotal: ${money(rememberedTotal,restaurant.currency)}`)}}><Copy size={16}/> Liste kopieren</button><button className="clear-list" onClick={()=>{setRemembered({});setListOpen(false)}}>Merkliste leeren</button></footer></aside></div>}
